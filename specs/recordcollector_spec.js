...
describe('Record Collector', function(){
  ...
  it('money increases when record collector sells a record', function(){
    ...
    recordcollector1.sellRecord(record2, recordstore1);
    assert.equal(93.5, recordcollector1.money);
  })

  it('record is transferred when record collector sells a record', function(){
    recordcollector1.addRecordToCollection(record1);
    recordcollector1.addRecordToCollection(record2);
    recordcollector1.sellRecord(record2, recordstore1);
    assert.deepEqual([{"artist": "Bon Iver", "price": 7.5, "title": "Bon Iver"}], recordcollector1.collection);
